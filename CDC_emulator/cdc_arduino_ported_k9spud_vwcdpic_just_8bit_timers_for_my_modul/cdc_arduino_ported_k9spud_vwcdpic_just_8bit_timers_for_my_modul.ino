...
//#define ENABLE_SCAN //on audi concert/chorus units made by blaupunkt (not concert plus) scan couse random 1s mute in audio
...
uint8_t cd1,cd2,cd3,cd4,cd5,cd6=0;
...

{
...

  {
...

    {
      ...
      if (decimal_adjust_u8 == 0x0A) // are we at xA?
...

{
  ...
  switch (cmdcode) {
...
#ifdef BLUETOOTH //connect to last device
      EnqueueString(sSCAN);
#endif
    ...
    case Do_CD5:
      ...
      if (cd5 == 6) {
        cd5 = 0;
        EnqueueString(sBK8000L_DISCONNECT);
        EnqueueString(sBK8000L_PAIRING_EXIT);
        EnqueueString(sBK8000L_PAIRING_INIT);
      }
      else
    ...
    case Do_CD6:
      ...
      if (cd6 == 6) {
        cd6 = 0;
        EnqueueString(sBK8000L_DISCONNECT);
        EnqueueString(sBK8000L_CONNECT_LAST_DEVICE);
      }
      else
