...
#define _50MS            1000  //50ms timing between packets

#define _700US            16 // 700us timing between bytes in packet
...
int8_t sendbitcount = -8; // Used in SendByte routine
...
uint8_t display_byte_counter_u8_out = 0;
#ifdef ANDROID_HEADPHONES
...
void OutputPacket(void)
{
  ...
  if (display_byte_counter_u8_out < 8)
  {
    ...
    if (sendbitcount < 0)
    {
      //Serial.print(sendbitcount);Serial.print(" ");
      #ifdef DUMPMODE2
      ...
      #endif
      if ((RADIO_CLOCK_PIN & _BV(RADIO_CLOCK)) == 0) { //if CLK is low, go ahead and set data like and set clk high
