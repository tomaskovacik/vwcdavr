...
cdc cdc(DataOut,false,DataIn,ClockPin);
...
//  Serial.println("start");
  ...
  pinMode(DataOut,INPUT); // signals from radio
  if (DataOut==2)
        attachInterrupt(0,cdc.read_Data_out,CHANGE);
  if (DataOut==3)
          attachInterrupt(1,cdc.read_Data_out,CHANGE);
  SPI.begin();
...
void loop(){
  ...
  {
    cdc.newcmd=0;
    uint8_t c = cdc.getCommand(cdc.cmd);
...
//    Serial.println(idle);
 ...
 if ((millis()-previousMillis)>PACKET_DALEY || DO_UPDATE){
  if(idle){
    cdc.send_package(0x74,0xFF^cd,0xFF^tr,0xFF,0xFF,mode,0x8F,0x7C);
  }
  else
  {
    cdc.send_package(0x34,0xFF^cd,0xFF^tr,0xFF,0xFF,mode,0xCF,0x7c);
  }
