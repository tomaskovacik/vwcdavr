...
void setup(){
  ...
  Serial.begin(115200);
  mySerial.begin(9600);
  mySerial.println("vw group cd changer emulator");
  cdc_setup(DataOut);
  //init RADIO:
  ...
  //  delay(10);
delay(2000);
//Serial.println("AT#CY");
//delay(500);
mySerial.println("AT#MP");
Serial.println("AT#CO");
delay(500);
  while (Serial.available()>0){
        mySerial.write(Serial.read());
  }
mySerial.println("AT#MG");
Serial.println("AT#MG");
delay(500);
  while (Serial.available()>0){
        mySerial.write(Serial.read());
  }
//mySerial.println("AT#MP");
//Serial.println("AT#MP");
//delay(500);
//  while (Serial.available()>0){
//        mySerial.write(Serial.read());
//  }
mySerial.println("AT#MQ");
Serial.println("AT#MQ");
delay(500);
  while (Serial.available()>0){
        mySerial.write(Serial.read());
  }
mySerial.println("AT#CC");
Serial.println("AT#CC");
delay(500);
  while (Serial.available()>0){
        mySerial.write(Serial.read());
  }
mySerial.println("AT#MI");
Serial.println("AT#MI");
delay(500);
  while (Serial.available()>0){
        mySerial.write(Serial.read());
  }
mySerial.println("AT#MV");
Serial.println("AT#MV");
delay(500);
  while (Serial.available()>0){
        mySerial.write(Serial.read());
  }
mySerial.println("AT#MO");
Serial.println("AT#MO");
delay(500);
  while (Serial.available()>0){
        mySerial.write(Serial.read());
  }
mySerial.println("AT#MA");
...
//      Serial.println(c,HEX);
      ...
      case CDC_END_CMD:
        ...
        else if (prevcmd==CDC_STOP)
        {
          ...
          Serial.println("AT#MA");
          //54BEFEFFFEFF8F5C
          send_package(0x54,0xBF^cd,0xFF^tr,0xFF,0xFF,mode,0x8f,0x5C);
          //previousMillis=millis();
          DO_UPDATE=1;
        }
        prevcmd=CDC_END_CMD;
  ...
  if ((millis()-previousMillis)>PACKET_DALEY || DO_UPDATE){
    if(idle){
        send_package(0x74,0xBF^cd,0xFF^tr,0xFF,0xFF,mode,0x8F,0x7C);
    }
    else
    {
